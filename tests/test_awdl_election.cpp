...
void test_add_valid_peer(awdl_peers_t peers, const struct ether_addr *addr) {
    struct awdl_peer *peer;
    awdl_peer_add(peers, addr, 0, NULL, NULL);
    awdl_peer_get(peers, addr, &peer);
    peer->is_valid = true;
}
...
TEST(awdl_election, elect_simple) {
    ...
    test_add_valid_peer(p.peers, &TEST_ADDR1);
...
TEST(awdl_election, elect_simple_invalid) {
    struct awdl_election_state s;
    struct awdl_peer_state p;
    awdl_election_state_init(&s, &TEST_ADDR0);
    awdl_peer_state_init(&p);
